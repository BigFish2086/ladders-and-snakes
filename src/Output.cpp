...
int Output::GetCellStartY(const CellPosition& cellPos) const
{
	///TODO: implement the following function as described in Output.h file
	int v = cellPos.VCell();
	int cellStartY = UI.ToolBarHeight + v * UI.CellHeight;
...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	pWind->GetIntegerSize(w, h, cardNum);//LOOK HERE AGAIN!!!!!!
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_VALUE] = "images\\Menu_InputValue.jpg";
...
void Output::PrintPlayersInfo(string info)
{
	if (UI.InterfaceMode == MODE_PLAY) {
		...
		pWind->SetPen(UI.ToolBarColor, 1);
		pWind->SetBrush(UI.ToolBarColor);
		pWind->DrawRectangle((PLAY_ITM_COUNT + 1) * UI.MenuItemWidth, 0, UI.width, UI.ToolBarHeight);
	}
	else {
		pWind->SetPen(UI.ToolBarColor, 1);
		pWind->SetBrush(UI.ToolBarColor);
		pWind->DrawRectangle((DESIGN_ITM_COUNT + 1) * UI.MenuItemWidth, 0, UI.width, UI.ToolBarHeight);
	}
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	if (!(playerNum >= 0 && playerNum <= (MaxPlayerCount - 1))) {
		return;
	}
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int numOfUsedCells = abs(fromStartY - toStartY) / UI.CellHeight;
	int y12 = 0;
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
	for (int i = 1; i <= numOfUsedCells; i++) {
		y12 = toStartY + i * UI.CellHeight;
		//std::cout << y12 << endl;
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	//vertical points
	int upperY = y1 - yChange;
	int lowerY = y1;
	int upperX = x12;
	//horizontal points
	int leftY = y1 - yChange / 2;
	int leftX = x12 - xChange;
	int rightX = x12 + xChange;
	//array of y coordinates
	int yCoords[4] = { upperY,leftY,lowerY,leftY };
	//array of x coordinates
	int xCoords[4] = { upperX,leftX,upperX,rightX };
	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way
	pWind->DrawPolygon(xCoords, yCoords, 4);
}
